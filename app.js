...
const formEl = document.querySelector(".lorem-form");
const amountEl = document.getElementById("amount");
const resultEl = document.querySelector(".lorem-text");

formEl.addEventListener("submit", function (e) {
  e.preventDefault();
  const value = parseInt(amountEl.value);
  const random = Math.floor(Math.random() * text.length);
  // validation for empty, < and >
  if (isNaN(value) || value <= 0 || value > 9) {
    ...
    const textMarkup = tempText
      .map(
        (text) => `
    <p class='result'>${text}</p>`
      )
      .join("");
    resultEl.innerHTML = textMarkup;
  }
});
